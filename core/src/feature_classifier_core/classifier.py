...
# Version of the neutral model bundle written by ``Classifier.export_bundle``.
# Bump when the bundle's keys or semantics change so ``load`` can refuse or
# upgrade bundles it does not understand.
...
class Classifier:
    ...
    def save(self, output_path):
        ...
        This is the working-session format used by the napari GUI: it pickles
        the entire object so annotation can be resumed losslessly. Because it
        pickles ``self``, the file is tied to this package's internals. For a
        portable, dependency-light model to hand to headless/downstream
        consumers, use :meth:`export_bundle` instead.
        """
    ...
    def export_bundle(self, output_path):
        """Write a neutral, versioned model bundle loadable with scikit-learn alone.

        The bundle is a plain dict
        ``{format_version, estimator, feature_names, class_names}`` and carries
        no training data, colors or GUI state. It can be consumed without
        importing this package (``joblib.load`` plus the fitted ``estimator``),
        and stays portable across plugin versions.
        """
        self.logger.info(f"Exporting model bundle at {output_path}...")
        joblib.dump(
            {
    ...
    def load(cls, path) -> "Classifier":
        """Load a classifier from either a neutral bundle or a full pickle.

        - A bundle written by :meth:`export_bundle` is reconstructed into a
          predict-capable (headless) ``Classifier`` with no training data.
        - A full-object pickle (a ``.clf`` written by :meth:`save`) is
          returned unchanged, preserving its training data and GUI state.
        """
        try:
            obj = joblib.load(path)
        except Exception:  # noqa: BLE001 - fall back to a plain pickle payload
            with open(path, "rb") as f:
                obj = pickle.load(f)
        if isinstance(obj, cls):
            return obj
        if isinstance(obj, dict) and "estimator" in obj:
            return cls._from_bundle(obj)
        raise ValueError(f"Not a recognized classifier file: {path!r}")
